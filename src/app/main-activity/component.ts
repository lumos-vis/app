...
export class MainActivityComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    switch (this.global.appLevel) {
      case "practice":
        ...
        break;
      case "live":
        this.global.appMode = "movies-w-year.csv";
        break;
    }
  ...
  initLumos() {
    ...
    d3.csv(fp).then(function (data: Array<Object>) {
      ...
      context.userConfig["originalDataset"].forEach((d) => {
        ...
        if (!d.hasOwnProperty("timesVisited")) d["timesVisited"] = 0;
        if (!d.hasOwnProperty("ratioTimesVisited")) d["ratioTimesVisited"] = 0;
            ...
            switch (layer) {
              ...
              case "Selection":
                ...
                // For categorical attributes
                  ...
                  if (
                    Object.values(selectedDataObj).reduce(context.utilsService.sum) > 0 &&
  ...
  /**
   * Re-draw vis when plot container `as-split-area` is re-sized.
   */
  onResized(event: ResizedEvent, resizeSource: string) {
    switch (resizeSource) {
      case "awarenessPanelColumn":
        // Awareness Panel
        this.setWidthsForAwarenessPanelVis();
        this.updateAwarenessPanel();
        // Main Vis
        initializePlotInstance(this, this.currentPlotType);
        this.updateVis();
        break;
      ...
      case "plotContainer":
        ...
        break;
      default:
        console.log(`invalid resize source: ${resizeSource}`);
        break;
    }
  }
